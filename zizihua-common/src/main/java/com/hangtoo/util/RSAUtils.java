...
public class RSAUtils {
	...
	private final static Logger log= Logger.getLogger(RSAUtils.class);
	...
	public static KeyPair generateKeyPair() throws Exception {
		try {
			...
			final int KEY_SIZE = 1024;// 没什么好说的了，这个值关系到块加密的大小，可以更改，但是不要太大，否则效率会低   4096;
			keyPairGen.initialize(KEY_SIZE, new SecureRandom());
			KeyPair keyPair = keyPairGen.generateKeyPair();

			log.info(keyPair.getPrivate());
			log.info((new BASE64Encoder()).encodeBuffer(keyPair.getPrivate().getEncoded()));
			log.info(keyPair.getPublic());
			log.info((new BASE64Encoder()).encodeBuffer(keyPair.getPublic().getEncoded()));
			
			KEY_PAIR = keyPair;
			
			log.info("n:"+RSAUtils.getModulus());
			log.info("e:"+RSAUtils.getPublicExponent());
